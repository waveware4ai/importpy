# run command >> python -m pytest -v ./importpy/tests
...
def hello():
    print(f"{__name__} ::: hello from [{__file__}]")
    pkg.remote0.hello()

def ping():
    print(f"{__name__} ::: ping from [{__file__}]")
    pkg.remote0.ping()
'''
    path = tmp_path / "test/test0.py"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(code)
    return str(path)

@pytest.fixture
def remote0(tmp_path):
    code = '''
import os, io, sys
from pkg import remote1
...
def ping():
    ...
    remote1.ping()
'''
    path = tmp_path / "test/pkg/remote0.py"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(code)
    return str(path)

@pytest.fixture
def remote1(tmp_path):
    code = '''
import os, io, sys

def hello():
    print(f"{__name__} ::: hello from [{__file__}]")
    from pkg.remote import remote2
    remote2.hello()
...
def ping():
    ...
    remote2.ping()
'''
    path = tmp_path / "test/pkg/remote1.py"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(code)
    return str(path)

@pytest.fixture
def remote2(tmp_path):
    code = '''
import os, io, sys
...
def ping():
    print(f"{__name__} ::: ping from [{__file__}]")
'''
    path = tmp_path / "test/pkg/remote/remote2.py"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(code)
    return str(path)

@pytest.fixture
def remote3(tmp_path):
    code = '''
import os, io, sys
...
def ping():
    print(f"{__name__} ::: ping from [{__file__}]")
'''
    path = tmp_path / "test/pkg/remote/remote3.py"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(code)
    return str(path)

def test_importpy_protocol_remote_file_import_as_module(test0, remote0, remote1, remote2, remote3):
    test0 = importpy("file://" + test0)
    test0.hello()
    test0.ping()
    assert hasattr(test0, 'hello')

def test_importpy_protocol_remote_file_import_as_package(test0, remote0, remote1, remote2, remote3):
    remote0_parents = importpy("file://" + os.path.dirname(remote0))
    from pkg import remote0
    remote0.hello()
    remote0.ping()
    assert hasattr(remote0, 'hello')
...
class CustomZipMetaFinder(AbstractMetaFinder):
    def __init__(self, uselazy:bool = True, as_finder_role:bool = True):
    ...
    def strip_type(self, url: str) -> str :
